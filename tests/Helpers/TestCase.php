...
abstract class TestCase extends \PHPUnit\Framework\TestCase
{
	 ...
	 */
	protected function assertTokenPointer($code, int $line, File $codeSnifferFile, int $tokenPointer = null)
	{
	...
	protected function findClassPointerByName(File $codeSnifferFile, string $name)
	{
	...
	protected function findConstantPointerByName(File $codeSnifferFile, string $name)
	{
		$tokens = $codeSnifferFile->getTokens();
		for ($i = 0; $i < count($tokens); $i++) {
			if ($tokens[$i]['code'] !== T_STRING || $tokens[$i]['content'] !== $name) {
	...
	protected function findPropertyPointerByName(File $codeSnifferFile, string $name)
	{
		$tokens = $codeSnifferFile->getTokens();
		for ($i = 0; $i < count($tokens); $i++) {
	...
	protected function findFunctionPointerByName(File $codeSnifferFile, string $name)
	{
		$tokens = $codeSnifferFile->getTokens();
		for ($i = 0; $i < count($tokens); $i++) {
			if ($tokens[$i]['code'] !== T_STRING || $tokens[$i]['content'] !== $name) {
	 ...
	 * @return int|null
	 */
	protected function findPointerByLineAndType(File $codeSnifferFile, int $line, $tokenCode)
	{
		$tokens = $codeSnifferFile->getTokens();
		for ($i = 0; $i < count($tokens); $i++) {
